...
测量“音频输出 → 麦克风”延迟的 Windows 工具（以麦克风接收为准），支持有线耳机/扬声器与蓝牙耳机。
...
2. 程序测出有线基准：`延迟`（loopback 抓取点 → 麦克风）、`含缓冲`（播放启动 → 麦克风）、`偏移 = 含缓冲 − 延迟`。
3. 之后每副设备按 Enter 测量：
   - 支持 loopback 且延迟合理 → 直接给出 `延迟`；
   - 否则（如蓝牙）→ 用 `延迟 = 含缓冲 − 偏移` 计算。
