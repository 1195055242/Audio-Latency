...
有效性判据（`measureRepeated`）：`significance ≥ 35` 且 `quality ≥ 0.05`。

## 引导式流程（index.js `guidedMeasure`）

1. **每副设备**（有线/蓝牙任意顺序）自动测 5 次：
   - 设备支持 loopback → 同一次播放同时得到 `延迟` 与 `含缓冲`，各取中位数；`偏移 = 含缓冲 − 延迟`（更新基准）。
   - 设备不支持 loopback → 5 次 `含缓冲` 中位数，`延迟 = 含缓冲 − 偏移`。
2. 汇总显示：`延迟` 为主，括号内列 `含缓冲`、`偏移`。
3. 循环询问“下一副”。
