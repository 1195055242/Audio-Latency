...
//   --min-significance <s> 峰值显著性阈值（默认 35，主判据）
...
选项:
  ...
  --rate <hz>         采样率（默认 48000）
  --min-quality <q>   波形相关辅助阈值 0..1（默认 0.05）
  --min-significance <s>  峰值显著性阈值（默认 35，主判据：录音中确有测试信号）
...
async function guidedMeasure(cfg, args) {
  ...
  const minSignificance = cfg.minSignificance ?? 35;
  ...
  console.log('==================================================');
  console.log('引导式测量：每副设备自动测 5 次取中位数');
  console.log('请把待测耳机/扬声器设为默认播放设备，并靠近麦克风。\n');
