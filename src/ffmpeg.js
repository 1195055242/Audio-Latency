...
export function startLoopbackRecording({ helperPath, outPath, duration, captureName, sampleRate = 48000 }) {
  ...
  // helper 输出：
  //   READY <qpcTicks> <freq>              采集已开始
  //   START_QPC <qpcTicks> FREQ <freq>     麦克风首帧 QPC（录音样本 0 对应时刻）
  const info = { readyPerf: null, readyTicks: null, freq: null, startTicks: null };
  ...
  child.stdout.on('data', (d) => {
    ...
    stdoutBuf = lines.pop();
    for (const line of lines) {
      const r = line.match(/^READY (\d+) (\d+)/);
      if (r) {
        info.readyTicks = Number(r[1]);
        info.freq = Number(r[2]);
        info.readyPerf = performance.now();
        if (readyResolve) readyResolve();
      }
      const s = line.match(/^START_QPC (\d+) FREQ (\d+)/);
      if (s) {
        info.startTicks = Number(s[1]);
        if (!info.freq) info.freq = Number(s[2]);
      }
    }
  });
  ...
  return { child, promise, waitReady, info };
}
